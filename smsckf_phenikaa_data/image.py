...
class ImageProcessor(object):
    ...
    def stareo_callback(self, stereo_msg):
        ...
        if self.is_first_img:
            ...
            t = time.time()
            self.track_features()
            print('Number of features after matching:', self.num_features['after_matching'])
    ...
    def initialize_first_frame(self):
        ...
        # self.draw_stereo_points(cam0_points, cam1_points)
    ...
    def track_features(self):
        ...
        print('Number of features before tracking:', self.num_features['before_tracking'])
        ...
        print('Number of features tracked:', len(curr_cam0_points), ' / ', track_inliers.sum())
        ...
        # self.draw_stereo_points(curr_tracked_cam0_points, curr_cam1_points)
        ...
        # self.draw_stereo_points(curr_matched_cam0_points, curr_matched_cam1_points)
        ...
        print(f'Tracking rate: {curr_feature_num} / {prev_feature_num} ')
    ...
    def stereo_match(self, cam0_points):
        ...
        inlier_markers = np.logical_and.reduce(
            [inlier_markers.reshape(-1), err < 10, disparity < 50])
    ...
    def draw_stereo_points(self, cam0_points, cam1_points, save_prefix='stereo_points'):
        """
        Vẽ các điểm cam0_points lên ảnh trái và cam1_points lên ảnh phải, lưu bằng matplotlib.
        """
        img0 = self.cam0_curr_img_msg.image.copy()
        img1 = self.cam1_curr_img_msg.image.copy()

        # Nếu ảnh là grayscale, chuyển sang RGB để vẽ màu
        if len(img0.shape) == 2:
        ...
        axs[0].imshow(img0)
        axs[0].set_title('cam0_points')
        for pt in cam0_points:
            x, y = pt
            axs[0].plot(x, y, 'go', markersize=4)
        axs[0].axis('off')

        axs[1].imshow(img1)
        axs[1].set_title('cam1_points')
        for pt in cam1_points:
            x, y = pt
            axs[1].plot(x, y, 'ro', markersize=4)
        axs[1].axis('off')

        plt.tight_layout()
        # Lưu file với timestamp nếu có
        ts = getattr(self.cam0_curr_img_msg, 'timestamp', None)
        if ts is not None:
            plt.savefig(f'stereo/{save_prefix}_{ts:.3f}.png')
        else:
            plt.savefig(f'stereo/{save_prefix}.png')
        plt.close(fig)
